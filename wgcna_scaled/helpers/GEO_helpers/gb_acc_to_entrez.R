gb_acc_to_entrez <- function(GSE_matrix, platform_object, annot_column){
  ...
  refseq_to_entrez_df <- read.table("helpers/annotation_files/refseq-to-entrez.tsv",
                                    sep = '\t', stringsAsFactors = F)
  
  m <- match(rownames(GSE_matrix), refseq_to_entrez_df[,3])
  
  GSE_matrix_df <- data.frame(GSE_matrix,stringsAsFactors =F) 
  GSE_matrix_df$entrez_id <- refseq_to_entrez_df[m, 2]
